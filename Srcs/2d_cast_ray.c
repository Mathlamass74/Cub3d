...
void	draw_line(t_data *d, int term_x, int term_y)
{
	int x = d->player.posx;
	int y = d->player.posy;
	while (x++ < term_x && y++ < term_y)
		mlx_pixel_put(d->mlx, d->win, x, y, GREEN);
}
...
void	draw_rays_dda(t_data *d, int p_pos_x, int p_pos_y, double player_angle)
{
	...
	while (i < 60)
	{
		...
		map_x = p_pos_x / TILE_SIZE;
		map_y = p_pos_y / TILE_SIZE;
		...
		delta_dist_x = sqrt(1 + (ray_dir_y / ray_dir_x) * (ray_dir_y / ray_dir_x));
		delta_dist_y = sqrt(1 + (ray_dir_x / ray_dir_y) * (ray_dir_x / ray_dir_y));
		...
		while (!hit)
		{
			...
			if (d->map[map_y][map_x] != '0')
				hit = 1;
			// Draw the ray
			mlx_pixel_put(d->mlx, d->win, map_x, map_y, GREEN);
			draw_line(d, map_x, map_y);
		}
	i++;
	}
}
